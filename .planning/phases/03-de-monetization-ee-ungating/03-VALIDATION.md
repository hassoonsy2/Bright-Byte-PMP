...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 03-01-01 | 01 | 1 | BILL-01 | T-03-01 | Billing route/nav/command entry points are absent | source/search | `git grep -n "billing-and-plans\\|settings/billing\\|BillingRoot\\|BillingActionsButton" -- apps/web packages -- ':!apps/web/.react-router' ':!apps/web/build'` | yes | pending |
| 03-01-02 | 01 | 1 | BILL-02, BILL-03 | T-03-02 | Edition badge cannot open a paid-plan modal and modal cluster has no consumers | source/search | `git grep -n "PaidPlanUpgradeModal\\|components/license\\|license/modal\\|TALK_TO_SALES_URL\\|SUBSCRIPTION_REDIRECTION_URLS\\|SUBSCRIPTION_WEBPAGE_URLS" -- apps/web packages -- ':!apps/web/.react-router' ':!apps/web/build'` | yes | pending |
| 03-01-03 | 01 | 1 | BILL-04, BILL-06 | T-03-03 | Upsell stubs are classified and either removed or left as real CE functionality | source/search | `git grep -n "MARKETING_PRICING_PAGE_LINK\\|MARKETING_PLANE_ONE_PAGE_LINK\\|Upgrade to\\|Plane Pro\\|Plane One\\|UpgradeBadge" -- apps/web packages -- ':!apps/web/.react-router' ':!apps/web/build'` | yes | pending |
| 03-01-04 | 01 | 1 | BILL-05 | T-03-04 | Plan/pricing and payment subscription data are removed with no dangling imports | source/search | `git grep -n "EProductSubscriptionEnum\\|IPaymentProduct\\|TBillingFrequency\\|PLANE_COMMUNITY_PRODUCTS\\|PLANS_COMPARISON_LIST\\|PLANE_PLANS" -- apps/web packages -- ':!apps/web/.react-router' ':!apps/web/build'` | yes | pending |
| 03-01-05 | 01 | 1 | BILL-04, BILL-06 | T-03-05 | Phase 3 UI paths do not call Plane cloud hosts | source/search | `git grep -n "plane\\.so\\|app\\.plane\\.so" -- apps/web packages -- ':!apps/web/.react-router' ':!apps/web/build' ':!packages/i18n/src/locales'` | yes | pending |
| 03-01-06 | 01 | 1 | BILL-01..BILL-06 | T-03-06 | Monorepo still builds, type-checks, and lints after de-monetization | integration | `pnpm build && pnpm check:types && pnpm check:lint` | yes | pending |
| 03-01-07 | 01 | 1 | BILL-06 | T-03-07 | AGPL headers and upstream license files remain untouched | compliance | `git diff -- LICENSE.txt COPYRIGHT.txt` plus header diff grep | yes | pending |
...
- [ ] Billing route/nav/member action residual grep returns no Phase 3 source hits.
- [ ] License modal, checkout, talk-to-sales, and subscription redirection residual grep returns no Phase 3 source hits.
- [ ] Upgrade/marketing residual grep returns no in-app upsell hits.
- [ ] Plan/pricing residual grep returns no live source consumers.
- [ ] Plane cloud-host residuals are either gone from Phase 3 UI paths or explicitly classified as later brand/deploy scope.
- [ ] `pnpm build && pnpm check:types && pnpm check:lint` is green.
- [ ] AGPL headers, `LICENSE.txt`, and `COPYRIGHT.txt` are unchanged.
